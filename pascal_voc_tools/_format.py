...
class Format():
    ...
    def check_devkit_format(self, root_dir):
        """
        ...
        for file_path in files:
            ...
            if 'VOC' == os.path.basename(file_path)[:3]:
                check_result = self.check_voc_format(file_path)
                if not check_result:
                    return check_result
        return check_result
    
    def check_voc_format(self, voc_path):
        """
    ...
    def gen_voc_format(self, ann_dir, jpeg_dir, save_dir):
        """
